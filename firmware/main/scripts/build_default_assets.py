...
def get_text_font_path(builtin_text_font, stackchan_fonts_path):
    """
    ...
    font_path = os.path.join(stackchan_fonts_path, 'cbin', font_name)
...
def get_emoji_collection_path(default_emoji_collection, stackchan_fonts_path):
    """
    Get the emoji collection path if needed
    Returns the emoji directory path or None if no emoji collection is needed
    
    Supports:
    - PNG emoji collections from stackchan_fonts (e.g., twemoji_32, twemoji_64)
    """
    if not default_emoji_collection:
        return None

    # Try PNG emoji collections (e.g., emojis_32, twemoji_64)
    emoji_path = os.path.join(stackchan_fonts_path, 'png', default_emoji_collection)
    if os.path.exists(emoji_path):
        return emoji_path

    print(f"Warning: PNG emoji collection directory not found: {default_emoji_collection}")
    return None
...
def main():
    ...
    if not args.esp_sr_model_path or not args.stackchan_fonts_path:
        ...
        if not args.stackchan_fonts_path:
            args.stackchan_fonts_path = os.path.join(project_root, "components", "stackchan_fonts")
    ...
    text_font_path = get_text_font_path(args.builtin_text_font, args.stackchan_fonts_path)
