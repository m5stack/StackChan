...
InefaSpeechBubble::InefaSpeechBubble(lv_obj_t* parent, lv_color_t primaryColor, lv_color_t secondaryColor,
...
InefaSpeechBubble::~InefaSpeechBubble()
{
    _text.reset();
    _bubble.reset();
    _arrow.reset();
...
void InefaSpeechBubble::setSpeech(std::string_view text)
{
    if (text.empty()) {
        clearSpeech();
        return;
...
void InefaSpeechBubble::clearSpeech()
{
    _text->setText("");
    setVisible(false);
}

void InefaSpeechBubble::setVisible(bool visible)
{
...
void InefaSpeechBubble::setTextFont(void* font)
{
    if (_text && font) {
